...
const styles: Record<string, CSSProperties> = {
  ...
  item: { borderBottom: '1px solid var(--rule)' },
  q: {
    width: '100%',
    textAlign: 'left',
...
export default function FAQ({ faqs }: { faqs: FaqItem[] }) {
  ...
  return (
    <section id="faq" data-screen-label="FAQ" style={styles.section}>
      <div className="wrap">
        <div className="faq-layout" style={styles.layout}>
          <div style={styles.side}>
            <div className="mono" style={styles.kicker}>FAQ</div>
            <h1 style={styles.title}>
              Questions,<br />answered <span style={{ fontStyle: 'italic', color: 'var(--amber)' }}>plainly</span>.
            </h1>
            <p style={styles.side_p}>
              Can't find your answer here? Email{' '}
              <a href="mailto:hello@renoxium.com" style={{ color: 'var(--amber)', textDecoration: 'none' }}>
                Raviouliti@renoxium.com
              </a>{' '}
            ...
            {faqs.map((f, i) => {
              ...
              return (
                <div key={f.id} style={styles.item}>
                  <button
                    style={{ ...styles.q, cursor: 'none' }}
                    onClick={() => setOpen(isOpen ? -1 : i)}
                    aria-expanded={isOpen}
                    className={`faq-q${isOpen ? ' is-open' : ''}`}
                  >
                    <span style={styles.num} className="faq-num">{String(i + 1).padStart(2, '0')}</span>
                    <span style={styles.qText}>{f.question}</span>
                    <svg
                      width="20"
                      height="20"
                      viewBox="0 0 20 20"
                      fill="none"
                      style={{ ...styles.caret, transform: isOpen ? 'rotate(45deg)' : 'rotate(0)' }}
                    >
                      <path d="M10 4v12M4 10h12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                    </svg>
                  </button>
                  <div style={{ ...styles.ans, maxHeight: isOpen ? 360 : 0, opacity: isOpen ? 1 : 0 }}>
                    <div style={styles.ansBody}>{f.answer}</div>
                  </div>
                </div>
              );
            })}
